...
public partial struct WorkerGatherSystem : ISystem
{
    ...
    public void OnUpdate(ref SystemState state)
    {
                     ...
                     .WithEntityAccess())
        {
            ...
            if (needFindNode)
            {
                ...
                {
                    Debug.LogWarning("[WorkerGather] Worker chưa được gán mỏ. Hãy select worker rồi right click vào ResourceNode.");
                    continue;
                }

                if (!nodeLookup.HasComponent(gather.ValueRO.TargetNode))
                {
                    Debug.LogWarning($"[WorkerGather] TargetNode {gather.ValueRO.TargetNode} không có ResourceNodeData");
            ...
            if (needFindDepot)
            {
                ...
                Debug.Log($"[WorkerGather] Tìm thấy ResourceDepot: {gather.ValueRW.TargetDepot}");
            }
            ...
            {
                Debug.LogError($"[WorkerGather] ResourceNode {nodeEntity} không có LocalTransform");
                continue;
            }

            if (!transformLookup.HasComponent(depotEntity))
            {
                Debug.LogError($"[WorkerGather] Depot {depotEntity} không có LocalTransform");
                continue;
            }
                ...
                case WorkerGatherState.GoingToNode:
                    {
                        ...
                        {
                            Debug.Log("[WorkerGather] Đã tới mỏ → chuyển sang Gathering");
        ...
        if (workerCount == 0)
        {
            Debug.LogWarning("[WorkerGather] Không có worker nào match query WorkerTag + WorkerGatherData");
        }
    }
    ...
    ref SystemState state)
    {
                     ...
                     .WithEntityAccess())
        {
            ...
            {
                Debug.Log("[WorkerGather] Skip node vì Amount <= 0");
                continue;
            }
